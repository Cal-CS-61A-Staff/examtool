...
export default function AdminApp() {
    ...
    const handleCourseSelect = async (course) => {
        setCourse(course);
        setExamList(await (await post("list_exams", { token: getToken(), course })).json());
    };

    const handleExamSelect = async (exam) => {
        setExamData(await (await post("get_exam", { token: getToken(), course, exam })).json());
    };
        ...
        <Container>
            ...
            {username && course && (
                <Row>
                    <Col>
                        Select an exam
                        <Form.Control as="select" onChange={(e) => handleExamSelect(e.target.value)}>
