...
@click.command()
@click.option("--exam", multiple=True, default=["cs61a-test-final"], help="The list of exam names. If it is just one exam, just include one. Use multiple exam flags to add new exams.")
...
def gradescope_autograde(exam, name_question, sid_question, course, assignment, assignment_title, email, password, emails, mutate_emails, create, target):
    """
    ...
    if create or assignment is None:
        ...
        grader.main(exam, target, name_question, sid_question, course, gs_assignment_id=assignment, gs_assignment_title=assignment_title, emails=emails, email_mutation_list=email_mutation_list)
    else:
